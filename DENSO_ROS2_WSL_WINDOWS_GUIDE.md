...
```bash
ros2 launch denso_robot_bringup denso_robot_bringup.launch.py model:=vs060 sim:=true tool:=effecteur_v1 ik_solver:=pick_ik
```
...
```bash
ros2 launch denso_motion_control motion_server.launch.py model:=vs060 sim:=true tool:=effecteur_v1 ik_solver:=pick_ik
```
